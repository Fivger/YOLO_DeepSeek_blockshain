<template>
  <div class="container">
    <div class="left-panel">
      <!-- 新增：气象传感器状态 -->
      <div class="meteor-status" :class="meteorData.status === 'connected' ? 'status-success' : 'status-error'">
        气象传感器状态：{{ meteorData.status }}
      </div>
...
const firstRow = ref([
  { icon: 'icon-daqiwendu', label: '室内温度', value: meteorData.value.temperature },
  { icon: 'icon-kongqishidu_kongqishidu', label: '空气湿度', value: meteorData.value.humidity },
...
const secondRow = ref([
  ...
  { icon: 'icon-guangzhaoqiangdu', label: '光照强度', value: meteorData.value.illumination },
]);
...
const apiKey = 'sk-57605fb4bd3b4f13859b92270bd35b8c'; // 请替换为您的DeepSeekAPI密钥

// HTTP拉取气象数据
const fetchMeteorData = () => {
  axios.get('http://localhost:5000/get_meteor_data')
    .then(res => {
      meteorData.value = res.data;
      // 同步更新UI数据
      firstRow.value[0].value = meteorData.value.temperature;
      firstRow.value[1].value = meteorData.value.humidity;
      secondRow.value[2].value = meteorData.value.illumination;
    })
    .catch(err => {
      console.error('拉取气象数据失败：', err);
    });
};
...
const initWebSocket = () => {
  ...
  socket.on('meteor_data', (data) => {
    meteorData.value = data;
    // 同步更新UI数据
    firstRow.value[0].value = meteorData.value.temperature;
    firstRow.value[1].value = meteorData.value.humidity;
    secondRow.value[2].value = meteorData.value.illumination;
...
onMounted(() => {
  fetchMeteorData();    // 页面加载时拉取一次
  initWebSocket();      // 初始化WebSocket
  setInterval(fetchMeteorData, 10000); // 每10秒兜底拉取
});
